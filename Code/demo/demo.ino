...
{
  if(!prevMode)//Nếu chế độ tự động này tắt
  {
  ...
  else //Chế độ prevMode đúng(cđ tự động)
  {
...
{
  if(prevMode !=  param.asInt())//Kiểm tra điều kiện nếu chế độ prevMode khác( thay đổi trạng thái)
  {
    prevMode =  param.asInt();//Dòng này gán giá trị mới của param cho biến. Điều này cập nhật giá trị trạng thái trước đó.
    currMode = prevMode ? "A" : "M";//Dòng này sử dụng toán tử ba ngôi để gán giá trị cho biến currMode. Nếu prevMode là đúng (true), currMode được gán giá trị "A", ngược lại, nó được gán giá trị "M"
    digitalWrite(ModeLed, prevMode);//  Nếu prevMode là HIGH, chân GPIO sẽ được bật; ngược lại, nó sẽ tắt
    controlBuzzer(500);//Gọi hàm điều khiển loa
    if(!prevMode && toggleRelay == HIGH)//Nếu prevMode là sai (false) và toggleRelay là HIGH, điều kiện này sẽ được thực hiện
    {
      digitalWrite(RelayPin, LOW);//Nó đặt trạng thái của chân GPIO đó thành LOW
      toggleRelay = LOW;//Dòng này gán giá trị LOW cho biến toggleRelay.
      Blynk.virtualWrite(VPIN_RELAY, toggleRelay);//gửi giá trị của toggleRelay lên ứng dụng Blynk
    }
   }   
}

void controlBuzzer(int duration)//khi hàm controlBuzzer được gọi với giá trị duration cụ thể, nó sẽ bật buzzer, chờ trong khoảng thời gian duration và sau đó tắt buzzer
{
...
void displayData(String line1 , String line2)//Hiển thị chế độ tự động (A) or thủ công (M)
{
...
void getMoisture()
{
  sensorVal = analogRead(SensorPin);// Độc giá trị cảm biến chân 34 lưu vào sensorVal
  if (sensorVal > (wetSoilVal - 50) && sensorVal < (drySoilVal + 50) )// Khoảng setup giá trị Moisture
  {
    ...
    Serial.print("Moisture Percentage: ");//int ra oled % độ ẩms
...
//buttonState: Trạng thái hiện tại của nút button2 (HIGH hoặc LOW).
{
  ...
  switch (eventType) {
    case AceButton::kEventReleased: //hằng số trong thư viện đại diện cho việc nhả nút nhấn 
      //Serial.println("kEventReleased");
