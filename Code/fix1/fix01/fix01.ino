...
int wetSoilVal = 930 ;  // Giá trị min khi đất ướt
int drySoilVal = 3000 ;  //giá trị max khi đất khô
...
void getMoisture()
{
  ...
  {
    moisturePercentage = map(sensorVal ,wetSoilVal, drySoilVal, 0, 100);//Ánh xạ giá trị cảm biến
    // Nuyên mẫu : map(value, fromLow, fromHigh, toLow, toHigh)
int moisturePercentageReal  = 100 - moisturePercentage;  
    // Print result to serial monitor
    Serial.print("Moisture Percentage: ");
    Serial.print(moisturePercentageReal);
    Serial.println(" %");
  }
  else
  {
...
{ 
  ...
  displayData(String(moisturePercentageReal) + " %", "T:" + String(temperature1) + " C,  H:" + String(humidity1) + " %  " + currMode);// Viết lên màn hình OLed
  
  Blynk.virtualWrite(VPIN_MoistPer, moisturePercentageReal); // Gửi dữ liệu moisture đến Pin ảo 
...
void controlMoist()
{
  if(prevMode)
  {
    if (moisturePercentageReal < (moistPerLow))
    {
    ...
    if (moisturePercentageReal > (moistPerHigh))
    {
      if (toggleRelay == HIGH)
      {
        controlBuzzer(500);
